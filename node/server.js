...
var HANDLERS = {
    ...
    "/raw": function (req, res) {
        var qs = url.parse(req.url, true).query;
        var parts = servicestack_client_1.splitOnFirst(qs["message"], " ");
        if (!parts[0].trim())
            return;
...
var handleFile = function (path, res) {
    if (path.startsWith("/"))
    ...
    fs.readFile(path, function (err, data) {
        ...
        var parts = servicestack_client_1.splitOnLast(path, ".");
        if (parts.length != 2)
...
var server = http.createServer(function (req, res) {
    var pathInfo = servicestack_client_1.splitOnFirst(req.url, "?")[0];
